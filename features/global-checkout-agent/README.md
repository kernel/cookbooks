...
6. **Checkout.** The winning browser (the same one that verified the price, so location-based pricing can't change underneath you) re-checks the total, fills the merchant's normal card fields with the aliases plus your separately supplied customer fields, answers any AI-agent disclosure via the page's real checkbox, and submits once. Meanwhile an observer polls the card item (`retrieve` with `wait`, `events` with `after`) and surfaces any approval action to you, the trusted human. For AgentCard the outgoing request is held until you approve.
