...
| [kernel-agent-browser](https://github.com/kernel/skills/tree/main/plugins/kernel-cli/skills/kernel-agent-browser) | Selector strategy, waits, iframe handling for the storefront-specific `TODO` sections, live view for the human confirmation step |
...
> Fill in the TODO selectors in `features/global-checkout-agent/index.ts` for `<storefront>`. Use the kernel-agent-browser skill to pick stable selectors, keep the vault alias handoff exactly as written, and do not add any retry around checkout submission.
