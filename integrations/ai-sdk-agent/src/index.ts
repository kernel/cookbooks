...
async function promptTaskSelection(): Promise<string> {
  const TASK_OPTIONS = {
    COINBASE:
      "Visit https://www.ycombinator.com/companies, search for Coinbase, and report the company's team size.",
    ONE_PIECE:
      "Visit https://onepiece.fandom.com/wiki/One_Piece_Wiki, find the 'On This Day' section, and tell me a fact from it.",
...
async function main() {
  ...
  console.log(
    boxen(chalk.cyan.bold("Welcome to Kernel AI SDK Agent!"), {
      padding: 1,
      margin: 1,
      borderStyle: "round",
      borderColor: "cyan",
  ...
  console.log(
    chalk.blue(`Starting agent to complete task: ${chalk.bold(`"${task}"`)}\n`),
  );
  ...
  browserSpinner.succeed(
    chalk.green(
      `Browser session created: ${chalk.underline(session.browser_live_view_url)}`,
    ),
  );
  console.log();
  ...
  try {
      ...
      tools: {
        execute_playwright: tool({
          ...
          outputSchema: z.object({
            success: z
              .boolean()
              .describe("Whether the code executed successfully"),
            error: z
              .string()
              .optional()
            ...
            result: z
              .unknown()
              .optional()
              .describe("The value returned by the code (if any)"),
            stderr: z
              .string()
              .optional()
              .describe("Standard error from the execution"),
            stdout: z
              .string()
              .optional()
              .describe("Standard output from the execution"),
            newSnapshot: z
              .string()
              .optional()
              .describe("The new snapshot of the page"),
          }),
          execute: async ({ code }) => {
            ...
            const newSnapshot = await kernel.browsers.playwright.execute(
              session.session_id,
              {
    ...
    const agentSpinner = ora(
      chalk.cyan("Agent is working on your task..."),
    ).start();
    ...
    // Use exitCode instead of process.exit() so the finally block below still
    // runs and the Kernel browser session is cleaned up on error.
    process.exitCode = 1;
  } finally {
    // Clean up browser session
    const cleanupSpinner = ora(
      chalk.cyan("Cleaning up browser session..."),
    ).start();
    try {
      ...
      if (error instanceof NotFoundError || (error as any)?.status === 404) {
        cleanupSpinner.succeed(
          chalk.yellow(
            `Session already cleaned up (ID: ${session.session_id})`,
          ),
        );
      } else {
