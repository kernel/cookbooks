...
import {
  mergeAndPersistProfileFields,
  readProfileFields,
} from "../mastra/memory/profile-memory.js";
import {
...
function isHelpRequested(args: ArgMap): boolean {
  return (
    args.help === true ||
    args.h === true ||
    process.argv.slice(2).some((v) => v === "help")
  );
}
...
async function runAgentTurnWithTrace(args: {
  ...
  for await (const chunk of stream.fullStream as AsyncIterable<any>) {
    ...
    if (type === "tool-call") {
      ...
      process.stdout.write(
        `[agent] tool-result ${String(payload.toolName ?? "unknown")}\n`,
      );
  ...
  const reasoningTextFromOutput =
    typeof (full as { reasoningText?: unknown }).reasoningText === "string"
      ...
      : "";
  const reasoningText =
    reasoningTextFromOutput || streamedReasoningText || undefined;
...
function extractNonSensitiveFieldsFromMessage(
  message: string,
): Partial<FormFields> {
  ...
  const fullNameMatch = text.match(
    /(?:my\s+name\s+is|i\s*am)\s+([a-z][a-z' -]{1,80})/i,
  );
  if (fullNameMatch?.[1]) {
  ...
  const companyMatch = text.match(
    /(?:company|organization)\s*(?:is|:)\s*([a-z0-9&'., -]{2,80})/i,
  );
  if (companyMatch?.[1]) {
  ...
  const phoneMatch = text.match(
    /(?:phone|mobile|cell)\s*(?:is|:)?\s*([\d +().-]{7,})/i,
  );
  if (phoneMatch?.[1]) {
  ...
  const locationMatch = text.match(
    /(?:location|city)\s*(?:is|:)\s*([a-z0-9'., -]{2,80})/i,
  );
  if (locationMatch?.[1]) {
    out.location = locationMatch[1].trim();
  }

  const postalCodeMatch = text.match(
    /(?:zip|postal(?:\s+code)?)\s*(?:is|:)?\s*([a-z0-9 -]{3,16})/i,
  );
...
function buildInteractiveWebTaskAgent(
  sessionId: string,
  model: string,
  seedUrl: string,
): Agent {
  ...
  const inspectPageFormTool = createTool({
    ...
    description:
      "Inspect the current page and extract visible form fields and submit controls.",
  ...
  const fillPageFormTool = createTool({
    id: "fill-page-form",
    description:
      "Fill one or more page inputs and attempt to submit the current form.",
    inputSchema: z.object({
      fills: z.array(
        z.object({
          selector: z.string(),
    ...
    execute: async ({ fills, submitSelector, pageUrl }) => {
      ...
      const result = await applyLoginFormValues(
        sessionId,
        fills,
        submitSelector,
        targetUrl,
...
async function runInteractiveWebTaskLoop(args: {
  ...
  const webTaskAgent = buildInteractiveWebTaskAgent(
    args.sessionId,
    model,
    args.startUrl,
  );
  ...
  let rememberedProfile = await readProfileFields(
    args.resourceId,
  ...
  try {
    for (let turn = 0; turn < args.maxTurns; turn += 1) {
      const preTurnSnapshot = await extractLoginFormFields(
        args.sessionId,
        args.startUrl,
      ).catch(() => undefined);
      ...
      const historyText =
        conversation.length > 0
          ? conversation
              .map(
                (entry, idx) =>
                  `${idx + 1}. ${entry.role.toUpperCase()}: ${entry.text}`,
              )
              .join("\n")
          : "(none)";
...
Always ground your answer in this snapshot and tools. If snapshot url is about:blank, call inspectPageFormTool with "${args.startUrl}" first.`,
      ...
      if (args.trace && (result.reasoning || result.reasoningText)) {
        ...
        if (result.reasoningText && !result.reasoningStreamed) {
          process.stdout.write(
            `[agent] reasoning full: ${result.reasoningText}\n`,
          );
        }
      }
      ...
      const fieldUpdates =
        extractNonSensitiveFieldsFromMessage(latestUserMessage);
...
async function runHitlWebTaskFlow(args: {
  ...
  try {
    ...
    replayUrl = await closeBrowserSession(
      session.sessionId,
      session.replayId,
    ).catch(() => undefined);
...
async function runStart(args: ArgMap): Promise<void> {
  ...
  if (!siteArg) {
    throw new Error(
      "Missing required --site. Pass a full URL, e.g. https://www.saucedemo.com/",
    );
  }
  const siteUrl = normalizeSiteInput(siteArg);
  if (!isHttpUrl(siteUrl)) {
    throw new Error(
      `--site must be a valid URL or domain. Received: ${siteArg}`,
    );
  }

  const resourceId = argString(args, "resourceId", "demo-user") ?? "demo-user";
  const profileThreadId =
    argString(args, "threadId", `${resourceId}-profile`) ??
    `${resourceId}-profile`;
  const task =
