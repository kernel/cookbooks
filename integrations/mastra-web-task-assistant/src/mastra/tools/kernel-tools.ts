...
// Lazily construct the client so importing this module (e.g. for --help)
// does not require KERNEL_API_KEY to be set.
let kernelClient: Kernel | undefined;
...
async function getReplayViewUrlWithRetry(
  sessionId: string,
  replayId: string,
...
async function executePlaywright<T>(
  sessionId: string,
  code: string,
): Promise<T> {
  ...
  if (!result.success) {
    throw new Error(
      `Playwright execution failed: ${result.error ?? "Unknown error"} ${result.stderr ?? ""}`.trim(),
    );
  }
...
export async function extractLoginFormFields(
  sessionId: string,
  url?: string,
): Promise<{
  url: string;
