...
- [Kernel CLI](https://www.kernel.sh/docs/reference/cli) installed
...
See the [Kernel Browser Pools documentation](https://www.kernel.sh/docs/browsers/pools) for more options.
...
### Running Against OpenAI Instead of OpenRouter (Optional)

If you do not have OpenRouter credits, you can point the agent at any OpenAI-compatible endpoint with `--base-url`. The endpoint is still authenticated with `OPENROUTER_API_KEY`, so set that variable to the key for your endpoint:

```bash
OPENROUTER_API_KEY=$OPENAI_API_KEY uv run python -m scripts.run_agent \
  --url https://github.com \
  --task "Navigate to the sign in page" \
  --max-steps 5 \
  --base-url https://api.openai.com/v1 \
  --model gpt-4.1-mini \
  --webjudge --webjudge-model gpt-4.1-mini
```

Notes:

- The agent sends `temperature=0.0` and `max_tokens`, which OpenAI's gpt-5.x models reject. Use a model that accepts both, such as `gpt-4.1-mini`. Expect weaker click grounding than the Qwen VLMs, since the action space uses Qwen's normalized 0-999 coordinates.
- WebJudge defaults to OpenRouter. Set `WEBJUDGE_BASE_URL` (for example `https://api.openai.com/v1`) to point it at another OpenAI-compatible endpoint, and pass a `--webjudge-model` the endpoint accepts with `temperature=0`, such as `gpt-4.1-mini`.
...
The Modal deployment scripts require the `modal` package, which is not part of the default dependencies. Install it with `uv sync --extra modal`.
...
- [Architecture Overview: Reward Models](./architecture.md#4-reward-models-corereward_models) - Configure and tune the WebJudge reward model
