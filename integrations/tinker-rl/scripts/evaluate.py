...
def _run_episode_sync(
    ...
    try:
        ...
        console.print(
            f"    {ts()} [dim]{task_label}: step=1 action=navigate time={t_nav:.1f}s url={task.initial_url}[/]"
        )
    ...
    if convo_id and is_raindrop_enabled():
        raindrop_on_step_start, raindrop_on_step_complete, raindrop_on_action_overlay, _ = (
            create_step_callbacks(
    ...
    def on_step_complete(step: int, step_result):
        ...
        if step_result.error or step_result.action is None:
            ...
            if step_result.is_terminal:
                timing = (
                    f"total={step_result.total_time:.1f}s predict={step_result.predict_time:.1f}s"
                )
            else:
                timing = f"total={step_result.total_time:.1f}s predict={step_result.predict_time:.1f}s exec={step_result.exec_time:.1f}s"
            console.print(
                f"    {ts()} [dim]{task_label}: step={display_step} {timing} action={desc}[/]"
            )
...
async def run_single_episode(
    ...
    try:
        ...
        # This runs completely independently of the main event loop
        ...
        if not reuse:
            console.print(
                f"    {ts()} [yellow]Releasing browser {adapter.session_id} with reuse=False (browser in bad state)[/]"
            )
        try:
            ...
            console.print(
                f"    {ts()} [red]Browser release failed for {adapter.session_id}: {e}[/]"
            )
...
async def eval_main(cfg: EvalConfig) -> int:
    ...
    avg_task_duration = sum(r["duration_seconds"] for r in results) / len(results) if results else 0
