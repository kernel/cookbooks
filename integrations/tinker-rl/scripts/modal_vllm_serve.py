...
def merge_lora_weights(
    ...
    if not lora_path.exists():
        raise ValueError(f"LoRA adapter not found at {lora_path}. Run upload_lora_adapter first.")
...
def serve():
    ...
    if merged_model_path.exists() and any(merged_model_path.glob("*.safetensors")):
        ...
        print(
            "  To use fine-tuned weights, run: modal run scripts/modal_vllm_serve.py::merge_lora_weights"
        )
    else:
        raise RuntimeError(f"No model found. Run prefetch_model or merge_lora_weights first.")
