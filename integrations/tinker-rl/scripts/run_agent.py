...
def parse_args() -> RunConfig:
    ...
    parser.add_argument(
        "--random", dest="random_task", action="store_true", help="Pick a random task"
    )
    parser.add_argument(
        "--task-file", default=None, help="Path to task JSONL file (overrides env default)"
    )
...
def load_task_from_file(
    ...
    if cfg.task_id:
        ...
        if not matching:
            ...
            raise ValueError(
                f"Task ID '{cfg.task_id}' not found. Available IDs (first 10): {available_ids}"
            )
...
def _create_step_callbacks(
    ...
    def on_step_complete(step: int, result: StepResult) -> None:
        ...
        if result.error or result.action is None:
            console.print(
                f"[yellow]Step {display_step}: {result.error or 'Failed to parse action'}[/]"
            )
            if step_interaction:
...
async def run_agent(
    ...
    try:
        ...
        if cfg.pool_name:
            ...
            kernel.browsers.delete_by_id(adapter.session_id)
