...
class AgentAuthEnv(Env):
    ...
    async def step(self, action: TinkerAction, *, extra: ActionExtra | None = None) -> StepResult:
        ...
        if browser_action is None:
            ...
            console.print(
                f"  {ts()} [red]{self.task.domain}: step={self.step_count} error=parse_failed[/]"
            )
        ...
        if getattr(browser_action, "is_terminal", False):
            ...
            console.print(
                f"  {ts()} [dim]{self.task.domain}: step={self.step_count} action={action_desc} [done][/]"
            )
        ...
        try:
            ...
            if not should_continue:
                ...
                console.print(
                    f"  {ts()} [dim]{self.task.domain}: step={self.step_count} exec={t_exec:.1f}s action={action_desc} [done][/]"
                )
            ...
            console.print(
                f"  {ts()} [red]{self.task.domain}: step={self.step_count} error=browser_failed[/]"
            )
            self.cleanup()
        ...
        if self.step_count >= self.config.max_steps:
            ...
            console.print(
                f"  {ts()} [yellow]{self.task.domain}: step={self.step_count} exec={t_exec:.1f}s action={action_desc} [max_steps][/]"
            )
        ...
        console.print(
            f"  {ts()} [dim]{self.task.domain}: step={self.step_count} exec={t_exec:.1f}s action={action_desc}[/]"
        )
    ...
    async def compute_group_rewards(
        ...
        for traj, env in zip(trajectory_group, env_group):
            ...
            try:
                ...
                if interaction is not None:
                    action_summary = (
                        " → ".join(env.action_history[-3:]) if env.action_history else "No actions"
                    )
                    finish_episode(
                        interaction=interaction,
